...
- Verify a pre-existing variable's actual behavior in code before coupling new logic to it, rather than trusting its name — caught a real bug where `behindCam` (pre-fork) silently conflated three unrelated culling reasons into one flag
- Stereo XR correctness requires per-eye matrices: `UNITY_MATRIX_VP` (Unity's ambient shader matrix) resolves correctly per-eye during stereo instanced rendering, but an explicit C# constant like `_MatrixVP` uploaded once per frame from a single mono `Camera.worldToCameraMatrix`/`GL.GetGPUProjectionMatrix` does not — it is the same value for both eyes. Never substitute a mono C#-uploaded matrix constant into any compute/vertex shader computation that must be per-eye-correct (clip-space position, view-space transforms). Caused a black-hole/warping regression in `CSCalcViewData` when `UNITY_MATRIX_VP` was swapped for `_MatrixVP` to fix an unrelated editor SceneView/GameView cross-talk issue — reproduced only in actual Quest/OpenXR stereo builds, not in editor even when forced to the Vulkan backend. Recurred a second time as a branch-hygiene issue, not a code regression: a long-lived feature branch forked from an old point on `main` and never picked up this exact fix, reproducing the identical flat/no-spatial-perception symptom in HMD. When diagnosing this symptom, check whether the branch's ancestry actually contains the fix commit before assuming the bug is new.
- `GaussianSplatRenderSystem.m_ActiveSplats` is a single shared List, not keyed per camera — `GatherSplatsForCamera` clears and repopulates this one list on every call regardless of which camera invoked it. In the editor, multiple cameras (including a fully disabled GameObject's camera — Unity's `ScriptableRendererFeature.OnCameraPreCull` fires for every registered camera on every GUI event, independent of `activeInHierarchy`) can call this within the same tick, and whichever call happens to run last silently overwrites the correct result. Any measurement or render output derived from `m_ActiveSplats` (cull counts, submitted-splat totals) is only trustworthy if this is fixed to be per-camera (e.g. keyed by camera instance ID) — not yet fixed as of 2026-07.
- A GPU output buffer (`GraphicsBuffer`/`RenderTexture`) that's allocated but never actually written by any compute dispatch is left holding whatever the fresh allocation happened to contain — GPU memory is not guaranteed zero-initialized. If that buffer is later read as real per-splat data (e.g. SH coefficients in a shading kernel), the result is visually-plausible-looking but wrong data that varies between allocations — attempt-to-attempt inconsistent corruption, not a deterministic bug. This is a different failure signature than a deterministic data/logic bug and is the tell to look for: if a symptom's pattern/severity changes between otherwise-identical runs at the same state, suspect an unwritten/uninitialized buffer before suspecting the data or algorithm feeding it. Also: when a code comment claims a buffer is populated "elsewhere" (e.g. "set at OnEnable via CopyBuffer"), verify that call actually exists in current source before trusting the comment — it can describe a mechanism that was silently removed or never implemented, leaving a stale, confidently-wrong description in place. Root-caused this way for `SplatMorph.compute`'s never-written `_OutSH` buffer (rainbow-colored splat corruption during morphing, 2026-07) via live GPU buffer introspection against a running Unity instance, not static source reading alone.
